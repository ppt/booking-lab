...
if ARGV.empty?
  ...
  t = Time.parse("00:05:00") - Time.now < 0 ? Time.parse("00:05:00") - Time.now + 24*60*60 : Time.parse("00:05:00") - Time.now
  puts "Sleep #{t} seconds"
  sleep(t)
end
...
def pptGetLogs(user, host)
  `#{$sshCmd} #{hostCmd(host)} ls /Users/#{user}/Dropbox/booking`.split("\n").select {|name|name.downcase.include? "#{host}" }
end
  ...
  if pcRunning?(user,host)
    ...
    for fname in pptGetLogs(user,host) do
      if host == 'pc'
        ...
        s = "scp  -q -o \"StrictHostKeyChecking=no\" -o \"UserKnownHostsFile=/dev/null\" #{hostCmd(host)}:/Users/#{user}/Dropbox/booking/#{fname} #{$dir_name}"
        # puts s
        `#{s}`        
      end
      # `ssh -q -o "StrictHostKeyChecking=no" -o "UserKnownHostsFile=/dev/null"  #{user}@ppt-#{host}.ddns.net rm #{fname}`
    end
    `cp logs/#{(Time.now - 86400).strftime("%d-%m-%Y")}/aws* #{$dir_name}`
