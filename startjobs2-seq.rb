...
def booking(host, user, passwd, course, seq, session)
  if host.downcase.include? 'azure'
    `ssh -q  -oUserKnownHostsFile=/dev/null -oStrictHostKeyChecking=no praphan@ppt-#{host}.ddns.net tmux new-session -d -s t#{session}`
    s = "ssh -q  -oUserKnownHostsFile=/dev/null -oStrictHostKeyChecking=no praphan@ppt-#{host}.ddns.net tmux send-keys -t t#{session} \"'docker run -it phan/virgin casperjs --user=#{user} --password=#{passwd} --seq='#{seq}' --class-name='#{course}' virgin-seq.js' Enter\""
    p s
    `#{s}`
    ...
    s = "ssh -q  -oUserKnownHostsFile=/dev/null -oStrictHostKeyChecking=no -i ~/Dropbox/booking/Docker/ntp.pem ec2-user@ppt#{host.downcase.gsub('aws','')}.ddns.net tmux send-keys -t t#{session} \"'docker run -it phan/virgin casperjs --user=#{user} --password=#{passwd} --seq='#{seq}' --class-name='#{course}' virgin-seq.js' Enter\""
    p s
    `#{s}`
  end
end
...
courses.each_with_index do |(key, value), index|
  ...
  value.each { |user, course, seq|
    ...
    booking key, user, passwd[user.to_s], course, seq, session
    session = session + 1
  }
end
