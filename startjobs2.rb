...
def booking(host, user, passwd, course, time, session)
  if host.downcase.include? 'azure'
    ...
    s = "ssh -q  -oUserKnownHostsFile=/dev/null -oStrictHostKeyChecking=no praphan@ppt-#{host}.ddns.net tmux send-keys -t t#{session} \"'docker run -it phan/virgin casperjs --user=#{user} --password=#{passwd} --class-time='#{time}' --class-name='#{course}' virgin.js' Enter\""
    p s
    `#{s}`
    ...
    s = "ssh -q  -oUserKnownHostsFile=/dev/null -oStrictHostKeyChecking=no -i ~/Dropbox/booking/Docker/ntp.pem ec2-user@ppt#{host.downcase.gsub('aws','')}.ddns.net tmux send-keys -t t#{session} \"'docker run -it phan/virgin casperjs --user=#{user} --password=#{passwd} --class-time='#{time}' --class-name='#{course}' virgin.js' Enter\""
    p s
    `#{s}`
  end
end
