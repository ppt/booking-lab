...
(function() {
  var casper, class_name, class_seq, class_time, delaytime, end_time, end_time_str, loopcnt, maxloop, moment, now, password, scan_time, sleep_time, test_flag, user, util;
    ...
    if (!this.exists('tr.virginRowStyle, tr.virginAltRowStyle')) {
      ...
      for (i = j = 0, len = rows.length; j < len; i = ++j) {
        ...
        if (t[3].toLowerCase().indexOf(class_name.toLowerCase()) >= 0) {
          this.echo(class_seq + ": " + t[0] + ", " + t[3] + ", " + t[5], 'INFO');
          if (class_seq === 1) {
            id = idtext[1];
            break;
          } else {
