...
function booking() {
  ...
  casper.then(function () {
    ...
    this.waitForSelector(
      '#modalBooking.modal.fade.ng-scope.in .modal-footer button[ng-click="vm.makeBooking()"]:not(.ng-hide)'
    );
  });
...
casper.start("https://mylocker.virginactive.co.th/", function () {
  ...
  if (!test_flag || (test_flag && casper.cli.has("start-time"))) {
    if (sleep_time > 0) casper.wait(sleep_time);
  }
});
...
casper.then(function(){
  logMsg("wait for bookaclass page to load");
  this.waitForSelector('a.ng-binding[href="#/bookaclass"]');
})

this.clickLabel("book a class or workout", "a");
this.click('a.ng-binding[href="#/bookaclass"]')
...
20019318,2046
  ...
  docker run -it phan/virgin3 casperjs virgin-seq.js --user=20010035 --password=1232 --class-name="Pump" --seq=2  --test-flag

  docker cp 72cf73a1ea0f:/home/virgin-seq.js /Users/praphan/Development/lab/booking-lab/virgin-seq-work.js
  ...
  if (this.exists('a.ng-binding[href="#/bookaclass"]')) {
    logMsg('Found Click');
  }
  this.click('a.ng-binding[href="#/bookaclass"]')
  ...
  // https://mylocker.virginactive.co.th/?aspxerrorpath=/www/login.aspx#/bookaclass
});

casper.then(function () {
  logMsg('after click #/bookaclass')
  logMsg(this.getCurrentUrl())
  logMsg('--------------------------------------------')
  logMsg(this.getHTML())
  logMsg('--------------------------------------------')
  this.waitForSelector("table.vaTable:not(.ng-hide)");
